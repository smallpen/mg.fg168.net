        ...
        <div class="py-1" role="none">
            ...
            @foreach($supportedLocales as $locale => $name)
                <button 
                    ...
                    @click="open = false"
                    class="group flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white {{ $isCurrentLanguage($locale) ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' : '' }}"
                    role="menuitem"
                >
                    {{-- 選中狀態指示器 --}}
