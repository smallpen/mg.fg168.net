<div class="card">
        ...
        <div class="flex items-center space-x-2">
            ...
            <div class="flex rounded-lg bg-gray-100 dark:bg-gray-700 p-1">
                <button 
                    ...
                    class="px-3 py-1 text-sm rounded-md transition-colors {{ $chartType === 'user_activity' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100' }}"
                >
                    使用者活動
                </button>
                <button 
                    wire:click="switchChart('role_distribution')"
                    class="px-3 py-1 text-sm rounded-md transition-colors {{ $chartType === 'role_distribution' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100' }}"
                >
                    角色分佈
                </button>
            </div>
    ...
    <div class="card-body">
        @if(!empty($chartData))
            <div class="relative">
                <!-- 圖表容器 -->
                <div class="h-64 flex items-center justify-center" id="chart-container-{{ $chartType }}">
                    @if($chartType === 'user_activity')
                        <!-- 線性圖表 (使用 CSS 模擬) -->
                            ...
                            @foreach($chartData['datasets'][0]['data'] as $index => $value)
                                ...
                                <div class="flex flex-col items-center flex-1">
                                    <div class="w-full bg-primary-500 rounded-t transition-all duration-300 hover:bg-primary-600" 
                                         style="height: {{ $height }}%"
                                         title="{{ $chartData['labels'][$index] }}: {{ $value }} 人">
                                    </div>
                    ...
                    @elseif($chartType === 'role_distribution')
                        <!-- 圓餅圖 (使用 CSS 模擬) -->
                                ...
                                @foreach($chartData['datasets'][0]['data'] as $index => $value)
                                    ...
                                    @if($percentage > 0)
                                        <div class="absolute inset-0 rounded-full border-8 border-transparent"
                                             style="border-color: {{ $colors[$index] ?? '#3B82F6' }}; 
                                ...
                                @foreach($chartData['labels'] as $index => $label)
                                    <div class="flex items-center space-x-2">
                                        <div class="w-3 h-3 rounded-full" 
                                             style="background-color: {{ $colors[$index] ?? '#3B82F6' }}">
                                        </div>
                            ...
                            @if($chartType === 'user_activity')
                                <div>
                                    ...
                                    <span class="font-medium text-gray-900 dark:text-gray-100">
                                        {{ $chartData['summary']['total'] }} 人
                                    </span>
                                </div>
                                @if($chartData['summary']['peak'])
                                    <div>
                                        <span class="text-gray-500 dark:text-gray-400">最高峰：</span>
                                        <span class="font-medium text-gray-900 dark:text-gray-100">
                                            {{ $chartData['summary']['peak']['label'] }} ({{ $chartData['summary']['peak']['count'] }} 人)
                                        </span>
                                    </div>
                                @endif
                            @elseif($chartType === 'role_distribution')
                                <div>
                                    ...
                                    <span class="font-medium text-gray-900 dark:text-gray-100">
                                        {{ $chartData['summary']['total_roles'] }} 個
                                    </span>
                                </div>
                                @if($chartData['summary']['largest_role'])
                                    <div>
                                        <span class="text-gray-500 dark:text-gray-400">最大角色：</span>
                                        <span class="font-medium text-gray-900 dark:text-gray-100">
                                            {{ $chartData['summary']['largest_role']['name'] }} ({{ $chartData['summary']['largest_role']['count'] }} 人)
                                        </span>
                                    </div>
                                @endif
                            @endif
